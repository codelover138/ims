...
function word_break(value) {
    let formattedEmail=value;
    if(formattedEmail)
    formattedEmail = formattedEmail.replace(/@/g, "@<wbr>").replace(/\./g, ".<wbr>");
...
function attachment_multiple(x) {
    if (x == null) {
        return '';
    }
    var formId = 'downloadForm_' + Math.random().toString(36).substring(7);
    return (
        '<div class="text-center">' +
        '<form id="' + formId + '" action="' + site.base_url + 'sales/download_multiple" method="post" style="display:none;">' +
        '<input type="hidden" name="attachments" value="' + x + '">' +
        '<input type="hidden" name="' + csrf_token_name + '" value="' + csrf_hash + '">' + // CSRF token, if needed
        '</form>' +
        '<a href="#" class="tip" title="' + lang.download + '" onclick="document.getElementById(\'' + formId + '\').submit(); return false;">' +
...
function row_status(x) {
    ...
    if (x == null) {
        return '';
    } else if (x.toLowerCase() == 'pending' || x.toLowerCase() == 'work in progress') {
        if(x =='Work in progress') x='wip';
        return '<div class="text-center"><span class="row_status label label-warning">' + lang[x] + '</span></div>';
    } else if (x.toLowerCase() == 'completed' || x.toLowerCase() == 'paid' || x.toLowerCase() == 'sent' || x.toLowerCase() == 'received') {
        return '<div class="text-center"><span class="row_status label label-success">' + lang[x] + '</span></div>';
    } else if (x.toLowerCase() == 'partial' || x.toLowerCase() == 'transferring' || x.toLowerCase() == 'ordered') {
        return '<div class="text-center"><span class="row_status label label-info">' + lang[x] + '</span></div>';
    } else if (x.toLowerCase() == 'due' || x.toLowerCase() == 'returned' || x.toLowerCase() == 'hold') {
...
$(document).ready(function() {
    ...
    $('body').on('click', '.transfer_link2', function() {
        $('#myModal').modal({ remote: site.base_url + 'transfers/view?id=' + $(this).attr('id') });
        $('#myModal').modal('show');
    });
